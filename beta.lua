...
luas2:Element("Button", "Serverhop", nil, function()
	local x = {}
	for _, v in ipairs(game:GetService("HttpService"):JSONDecode(game:HttpGetAsync("https://games.roblox.com/player/games/" .. game.PlaceId .. "/servers/Public?sortOrder=Asc&limit=100")).data) do
...
-------------------------------------------------------------------------------------------------------------------------------
local aimbot3 = rage123123213213:Tab('bloxsense.gay rage')
aimbot3:Element("Toggle", 'enabled')
aimbot3:Element("Toggle", 'anti aimbot correction')
aimbot3:Element("Toggle", 'force headshots')
aimbot3:Element("Toggle", 'force body aim')
aimbot3:Element("Toggle", 'automatic penetration')
aimbot3:Element("Toggle", 'silent aim')
aimbot3:Element("Toggle", 'force hit')
aimbot3:Element("Dropdown", 'prediction', {options = {'Off', 'Manual', 'Automatic', 'Test'}})
aimbot3:Element("Slider", 'Multiplier', {min = 0, max = 100, default = 1})
aimbot3:Element("ToggleKeybind", 'double tap')
--aimbot3:Element("Dropdown", 'ticks per second', {options = {"4", "8", "16", "32", "64"}})
-------------------------------------------------------------------------------------------------------------------------------
...
local Folder = INST("Folder")
	...
	local funcs = {}
	function createbullettracer(player, v2)
...
---------------------------------------------------bloxsense.gay
            function realWait(time12312321313)
                local OldTick12313123213213321321 = tick()
                time12312321313 = time12312321313 or 0
                repeat
                    game:GetService("RunService").Heartbeat:Wait()
                until tick() - OldTick12313123213213321321 >= time12312321313
                return tick() - OldTick12313123213213321321
            end
			function coolDown()
                if TBLFIND(values.misc.client["gun modifiers"].Jumbobox, "firerate") then
                    shootCooldown = false
                    ...
                    if game.ReplicatedStorage.Weapons:FindFirstChild(tostring(Client.gun)) then
                        realWait(game.ReplicatedStorage.Weapons:FindFirstChild(tostring(Client.gun)).FireRate.Value * 0.92)
                        shootCooldown = false
                    else
                        shootCooldown = false
                    end
            ...
            local function ArmorPenetration(player)
                local v2 = 1
                if player:FindFirstChild("Kevlar") then
                    if string.find(player.Name, "Head") then
                        if player:FindFirstChild("Helmet") then
                            v2 = v2 / 100 * Client.gun.ArmorPenetration.Value
                        end
                    else
                        v2 = v2 / 100 * Client.gun.ArmorPenetration.Value
                    end
...
local function RageFunction(RagePlayer123132, RageHitPart1212312, v6, v7)
                if RagePlayer123132.Character:FindFirstChildOfClass("ForceField") then
                            ...
                            ty = v6
                            if values.rage.rages['bloxsense.gay rage'].prediction.Dropdown == 'Off' then
                                tv = Vector3.new(0, 0, 0)
                            else
                                --script.Parent.Parent.Parent.Parent.Parent.Parent.Parent.tags.LC.TextColor3 =
                                    --Color3.fromHSV(0, 160 / 255, 214 / 255)
                                    
                                local v8 = values.rage.rages['bloxsense.gay rage'].Multiplier.Slider
                                ...
                                for PredictedPlayer, predictPos in pairs(sV) do
                                    if RagePlayer123132.Name == predictPos[1] then
                                        ...
                                        if vc.magnitude > 22 then
                                            ...
                                            if values.rage.rages['bloxsense.gay rage'].prediction.Dropdown == "Automatic" then
                                                local vf = game.Stats.PerformanceStats.Ping:GetValue()
                                                tv = vc.Unit * vc.Magnitude * vf / 1000 * (vf > 200 and 1.5 or 2)
                                            elseif values.rage.rages['bloxsense.gay rage'].prediction.Dropdown == "Test" then
                                            ...
                                            --script.Parent.Parent.Parent.Parent.Parent.Parent.Parent.tags.LC.TextColor3 =
                                                --Color3.fromHSV(0.3495, 0.626169, 0.839216)
                                        end
                                    end
                                end
                            end
...
local function WallBangCheck(fromLocalPlayer, toTarget)
                ...
                local GunRange, GunPen
                if Client.gun:FindFirstChild("Range") then
                    GunRange = Client.gun.Range.Value * 0.0694
                end
                if Client.gun:FindFirstChild("Penetration") then
                    GunPen = Client.gun.Penetration.Value * 0.01
                end
                local Distance = (fromLocalPlayer.p - toTarget.HumanoidRootPart.Position).Magnitude
                local ui = CFrame.new(fromLocalPlayer.p, fromLocalPlayer.p + fromLocalPlayer.LookVector).LookVector.Unit * math.clamp(GunRange, 8, Distance + 8)
                local uj = Ray.new(fromLocalPlayer.p, ui)
                local Hits = 0
                local PenRequired = 0
                local hit, pos, whatSideOfPart
                local WallBang = 1
                local uq = 1
                repeat
                    hit, pos, whatSideOfPart = workspace:FindPartOnRayWithIgnoreList(uj, IgnoreList, false, true)
                    if hit and hit.Parent then
                        ...
                        if
                            hit.Material == Enum.Material.CorrodedMetal or hit.Material == Enum.Material.Metal or
                                hit.Material == Enum.Material.Concrete or
                                hit.Material == Enum.Material.Brick
                         then
                            WallBang = 2
                        end
                        if
                            hit.Name == "Grate" or hit.Material == Enum.Material.Wood or
                                hit.Material == Enum.Material.WoodPlanks or
                                hit and hit.Parent and hit.Parent:FindFirstChild("Humanoid")
                         then
                            WallBang = 0.1
                        end
                        if
                            hit.Transparency == 1 or hit.CanCollide == false or hit.Name == "Glass" or
                                hit.Name == "Cardboard" or
                                hit:IsDescendantOf(workspace.Ray_Ignore) or
                                hit:IsDescendantOf(workspace.Debris) or
                                hit and hit.Parent and hit.Parent.Name == "Hitboxes"
                         then
                            WallBang = 0
                        end
                        if hit.Name == "nowallbang" then
                            WallBang = 100
                        end
                        if hit:FindFirstChild("PartModifier") then
                            WallBang = hit.PartModifier.Value
                        end
                        local hit2, pos2 = workspace:FindPartOnRayWithWhitelist(Ray.new(pos + ui * 1, ui * -2), {hit}, true)
                        local ut = (pos2 - pos).magnitude
                        ut = ut * WallBang
                        PenRequired = math.min(GunPen, PenRequired + ut) -- returns if pen of gun is bigger than required pen to wallbang
                        local noWalls = false
                        if Hits >= 1 then
                            noWalls = true
                        end
                        if
                            hit and hit.Parent and hit.Parent.Name == "Hitboxes" or
                                hit and hit.Parent.className == "Accessory" or
                                hit and hit.Parent.className == "Hat" or
                                hit.Name == "HumanoidRootPart" and hit.Parent.Name ~= "Door" or
                                hit.Name == "Head" and hit.Parent:FindFirstChild("Hostage") == nil
                         then
                        else
                            if hit and hit:IsDescendantOf(toTarget) then
                                return hit, pos, uq, noWalls
                            end
                        end
                        if WallBang > 0 then
                            Hits = Hits + 1
                        end 
                        uq = 1 - PenRequired / GunPen
                        if
                            hit and hit.Parent and hit.Parent.Name == "Hitboxes" or
                                hit and hit.Parent and hit.Parent.Parent and hit.Parent.Parent:FindFirstChild("Humanoid2") or
                                hit and hit.Parent and hit.Parent:FindFirstChild("Humanoid2") or
                                hit and hit.Parent and hit.Parent:FindFirstChild("Humanoid") and
                                    (1 > hit.Transparency or hit.Name == "HeadHB") and
                ...
                until hit == nil or PenRequired >= GunPen or Hits >= uv or 0 >= uq
            end
...
RunService.Stepped:Connect(
                        function()
                            ...
                            pcall(
                                function()
                                    gunDamage = Client.gun.DMG.Value or 1
                                end
                            )
                            if TBLFIND(values.rage.weapons.default.hitboxes.Jumbobox, 'head') and not values.rage.rages['bloxsense.gay rage']['force body aim'].Toggle then
                                table.insert(Hitboxes, "HeadHB")
                            end
                            if TBLFIND(values.rage.weapons.default.hitboxes.Jumbobox, 'torso') then
                                table.insert(Hitboxes, "LowerTorso")
                                table.insert(Hitboxes, "UpperTorso")
                            end
                            if TBLFIND(values.rage.weapons.default.hitboxes.Jumbobox, 'arms') then
                                table.insert(Hitboxes, "RightUpperArm")
                                table.insert(Hitboxes, "LeftUpperArm")
                            end
                            if TBLFIND(values.rage.weapons.default.hitboxes.Jumbobox, 'legs') then
                                table.insert(Hitboxes, "RightLowerLeg")
                                table.insert(Hitboxes, "LeftLowerLeg")
                                table.insert(Hitboxes, "LeftFoot")
                                table.insert(Hitboxes, "RightFoot")
                            end
                            if values.rage.rages['bloxsense.gay rage'].enabled.Toggle and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                                local LocalHRPCFrame = LocalPlayer.Character.HumanoidRootPart.CFrame.p + Vector3.new(0, 1.4, 0)
                                for i, v in next, Players:GetPlayers() do
                                    if
                                        LocalPlayer:FindFirstChild("Status") and v:FindFirstChild("Status") and
                                            v.Status.Team.Value ~= LocalPlayer.Status.Team.Value and
                                            v.Character and
                                            v.Character:FindFirstChild "UpperTorso" and
                                            v.Character.UpperTorso:FindFirstChild "Waist"
                                     then
                                        v.Character.UpperTorso.Waist.C0 =
                                            CFrame.new(Vector3.new(0, 0.6, 0)) * CFrame.Angles(0, 0, 0)
                                    end
                                            if
                                                v and v ~= LocalPlayer and v.Character and
                                                    v.Character:FindFirstChild("HumanoidRootPart") and
                                                    v.TeamColor ~= LocalPlayer.TeamColor
                                             then
                                                for vA, vB in next, Hitboxes do
                                                    if vB == "HeadHB" then
                                                        if not v.Character:FindFirstChild(vB) or values.rage.rages['bloxsense.gay rage']['force body aim'].Toggle then
                                                    ...
                                                    if v.Character:FindFirstChild(vB) then
                                                        ...
                                                        if HitPart12312312 then
                                                            ...
                                                            if
                                                                gunDamage * MODIFICATION * vF * ArmorPenetration(v) > MinDamage or
                                                                    gunDamage * MODIFICATION * vF * ArmorPenetration(v) >
 ...
 RunService.RenderStepped:Connect(
                        function(vl)
                            if values.rage.rages['bloxsense.gay rage'].enabled.Toggle then
                                for vm, vn in pairs(Players:GetPlayers()) do
                                    if
                                        vn and vn ~= LocalPlayer and vn.Character and
                                            vn.Character:FindFirstChild("HumanoidRootPart") and
                                            vn.TeamColor ~= LocalPlayer.TeamColor
                                     then
                                        pcall(
                                            function() --prediction ig
                                                local vo = nil
                                                local HumanoidRootPart = vn.Character.HumanoidRootPart
                                                local HumanoidRootPartPos = vn.Character.HumanoidRootPart.Position
                                                local oldTick = tick()
                                                RunService.RenderStepped:Wait()
                                                RunService.RenderStepped:Wait()
                                                vo = (HumanoidRootPart.Position - HumanoidRootPartPos) / (tick() - oldTick)
                                                local vs = {vn.Name, vo}
                                                if vo ~= nil then
                                                    sV = {}
                                                    table.insert(sV, vs)
...
if not getgenv().PasteDisabled then
	...
	RageTarget2 = RageTarget
		...
		if values.rage.rages['default rage']["prediction"].Dropdown ~= "off" and RageTarget ~= nil and values.rage.rages['default rage']["enabled"].Toggle then
			coroutine.wrap(function()
				if Players:GetPlayerFromCharacter(args[1].Parent) or args[1] == RageTarget then
					if values.rage.rages['default rage']["prediction"].Dropdown == "cframe" then
						local Velocity = (RageTarget.Parent.HumanoidRootPart.Position - RageTarget.Parent.HumanoidRootPart.OldPosition.Value)/LastStep
		...
		if values.rage.rages['bloxsense.gay rage']['enabled'].Toggle and TargettedPlayer then
            ...
            args[4] = 0
            args[12] = args[12] - 512
            if values.rage.rages['bloxsense.gay rage']['force hit'].Toggle then
                args[1] = RageTarget2
				args[8] = ty
                args[10] = tx
			end		
			if values.rage.rages['bloxsense.gay rage']['force headshots'].Toggle then
				args[1] = TargettedPlayer.Character.Head
			end
		end
		--[[
		1. args[1] is a part
