...
local create = {
    Dropdown = function(parent,choices,default,callback,min,max,limit)
        ...
        local function update(index,shouldSet)
            ...
            if not (holder and name) then return end
            ...
            for i,v in pairs(optionss) do
               local holder = utility:Draw('Square',{
                ...
                holder.MouseEnter:Connect(function()
                    t.Color = Themes[Theme].Ascent
                    TabInsert(ThemeDrawings.Ascent,t)
                    TabRemove(ThemeDrawings.InActiveText,t)
                    TabRemove(ThemeDrawings.Text,t)
                end)
                holder.MouseLeave:Connect(function()
                    TabRemove(ThemeDrawings.Ascent,t)
                    if not table.find(chosen,t.Text) then
                        t.Color = Themes[Theme].InActiveText
                        TabInsert(ThemeDrawings.InActiveText,t)
                    else
                        t.Color = Themes[Theme].Text
                        TabInsert(ThemeDrawings.Text,t)
        ...
        local out = utility:DoubleOutline(dropdown,Themes[Theme].Border,fromrgb(10,10,10))
        TabInsert(ThemeDrawings.Border,out)
        local out = utility:DoubleOutline(options,Themes[Theme].Border,fromrgb(10,10,10))
        TabInsert(ThemeDrawings.Border,out)
    ...
    Scroll = function(parent,choices,default,callback,limit)
        ...
        for i,v in pairs(choices) do
            ...
            Textholder.MouseEnter:Connect(function()
                text.Color = Themes[Theme].Ascent
                TabInsert(ThemeDrawings.Ascent,text)
                TabRemove(ThemeDrawings.InActiveText,text)
                TabRemove(ThemeDrawings.Text,text)
            end)
            Textholder.MouseLeave:Connect(function()
                TabRemove(ThemeDrawings.Ascent,text)
                if choice.text == v then
                    text.Color = Themes[Theme].Text
                    TabInsert(ThemeDrawings.Text,text)
                else
                    text.Color = Themes[Theme].InActiveText
                    TabInsert(ThemeDrawings.InActiveText,text)
                end
            end)
        ...
        local out = utility:DoubleOutline(scroll,Themes[Theme].Border,fromrgb(10,10,10))
        TabInsert(ThemeDrawings.Border,out)
        return choicesHolder,scrollFuncs
    end,
    Box = function(holder,placeholder,callback,finalcallback)
        local boxFuncs = {}
        local callback = callback or function() end
        local finalcallback = finalcallback or function() end
        ...
        function boxFuncs:set(text)
            ...
            if text == '' then
                ...
                TabInsert(ThemeDrawings.InActiveText,BoxPlaceholder)
                TabRemove(ThemeDrawings.Text,BoxPlaceholder)
                return
            end
            BoxPlaceholder.Text = text
            callback(BoxPlaceholder.Text)
        end
        function boxFuncs:backspace()
        ...
        local c;c=utility.connect(box.MouseButton1Click,function()
            if not c then utility.disconnect(c) end
            ...
            userinputLoop = utility.connect(game.UserInputService.InputBegan,function(input)
                ...
                elseif input.UserInputType == Enum.UserInputType.MouseButton1 then
                    finalcallback(BoxPlaceholder.Text,BoxPlaceholder.Color == placeholderColor)
...
local function SetupSection(section,update,sectorsHolder)
    ...
    function sectionFuncs:Toggle(options)
        ...
        function togglefuncs:ColorPicker(options)
            ...
            if #colorpickers ~= 0 then
                button.Position = UDim2.new(1,-((20*#colorpickers)+15),0,0)
            end
            TabInsert(colorpickers,button)
            ...
            local huepicker = utility:Draw('Square',{
                ...
                ZIndex = pickerHandler.ZIndex+2,
                Filled = true,
                Position = UDim2.new(0,4,0,163),
                Size = UDim2.new(0,200,0,11),
                Color = Themes[Theme].Border
            })
            utility:Outline(huepicker,fromrgb())
            ...
            local alphaframe = utility:Draw('Square',{
                ...
                ZIndex = pickerHandler.ZIndex+2,
                Filled = true,
                Position = UDim2.new(0,192,0,5),
                Size = UDim2.new(0,11,0,150),
            ...
            local o = utility:DoubleOutline(alphaframe,Themes[Theme].Border,fromrgb())
            TabInsert(ThemeDrawings.Object,o)
            -----------------------
            local set
            -----------------------
            local txt = utility:Draw('Square',{
                Parent = pickerHandler,
                Size = UDim2.new(0,198,0,5),
            ...
            -----------------------
            local hue,saturation,val = default:ToHSV()
            local alpha = 0

            local txtFuncs = create.Box(txt,'Color',function() end, function(color) 
                local tbl = string.split(color,',')
                if #tbl ~= 3 then
                    set(Color3.fromHSV(hue,saturation,val))
                    return
                end
                if not (tonumber(tbl[1]) and tonumber(tbl[2]) and tonumber(tbl[3])) then
                    set(Color3.fromHSV(hue,saturation,val))
                    return
                end
                tbl[1] = math.clamp(math.floor(tonumber(tbl[1])),0,255)
                tbl[2] = math.clamp(math.floor(tonumber(tbl[2])),0,255)
                tbl[3] = math.clamp(math.floor(tonumber(tbl[3])),0,255)

                set(Color3.fromRGB((tbl[1]), (tbl[2]), (tbl[3])),true)
            end)
            
            set = function(color,modifyPos)
                local hue2,saturation2,val2 = color:ToHSV()
                if modifyPos then
                    hue = hue2
                    saturation = saturation2
                    val = val2
                    colorPointer.Position = UDim2.new(saturation2, 0, 1-val2, 0)       
                    huePointer.Position = UDim2.new(hue2, 0, 0, 1)
                end
                ...
                txtFuncs:set(('%s, %s, %s'):format(tostring(r),tostring(g),tostring(b)))

                local clr = {Color = color,Transparency = alpha}
                library.flags[flag] = clr
                callback(clr)
            end
            set(default,true)
    ...
    function sectionFuncs:Dropdown(options)
        ...
        local funcs,dropdownfuncs = create.Dropdown(holder,dropdownOptions,default,function(value) library.flags[flag] = value rawcallback(value) end,min,max)
    ...
    function sectionFuncs:ScrollDrop(options)
        ...
        local updateFuncs = {}
        local Dropdownfuncs,_,dropdown = create.Dropdown(holder,indexNames,indexNames[1],function(choice)
    ...
    function sectionFuncs:Box(options)
        ...
        local funcs = create.Box(holder,placeholder,function(t)
...
local function SetupBorder(section,name)
...
function library:init(options)
    ...
    local frame = utility:Draw('Square',{
        Thickness = 0,
    ...
    function library:Tab(name)  
        ...
        function sectorsFuncs:MultiSection(options)
            ...
            TabInsert(ThemeDrawings.Sectors,section)
            function multiSectionFuncs:AddSection(options)
                local options = utility.table(options or {})
                local name = options.name or ''
                ...
                if not currentTab then
                    currentTab = section
                    holder.Size = UDim2.new(1,0,0,section.AbsoluteSize.Y)
                    section.Visible = true
                    local old = holder.Parent.Position    
                    holder.Parent.Position = UDim2.new()
                    holder.Parent.Position = old 
                end
                sections[name] = section
                return SetupSection(section,function() 
            ...
            function multiSectionFuncs:ChangeSection(name)
                ...
                currentTab = sections[name]
                currentTab.Visible = true
                holder.Size = UDim2.new(1,0,0,currentTab.AbsoluteSize.Y)
                local old = holder.Parent.Position    
                holder.Parent.Position = UDim2.new()
                holder.Parent.Position = old 
            end
            return multiSectionFuncs
        end
