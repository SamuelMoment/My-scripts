...
--[[
local ohTable1 = {
	["HitPart"] = workspace.Terrain,
	["EndPoint"] = -1658.2930908203125, -33.54548645019531, 1609.467041015625,
	["HitCallback"] = "Bullet",
	["BulletOwner"] = game:GetService("Players").LocalPlayer,
	["Lifetime"] = 1,
	["Speed"] = 80,
	["cframe"] = CFrame.new(-1657.96948, -25.2302818, 1637.18726, 0.999927759, 0.0044465675, 0.0111800013, -0.00747155584, 0.95781076, 0.287302762, -0.00943081267, -0.287365526, 0.957774639),
	["Tool"] = workspace["34aohrbzaa8nh56ozas4"]["Colt .45"],
	["Normal"] = 0.08802716434001923, 0.98614501953125, -0.140602707862854,
	["AmmoType"] = "PistolAmmo",
	["GunType"] = "Pistol",
	["Material"] = Enum.Material.Sandstone,
	["ToolName"] = "Colt .45",
	["StartPoint"] = CFrame.new(-1657.96948, -25.2302818, 1637.18726, 0.999927759, 0.0044465675, 0.0111800013, -0.00747155584, 0.95781076, 0.287302762, -0.00943081267, -0.287365526, 0.957774639),
	["MaxDistance"] = 1000,
	["Shot"] = .Shot,
	["HitPosition"] = -1658.2930908203125, -33.54548645019531, 1609.467041015625,
	["FirstFrame"] = true,
	["StartTime"] = 1666946759.8379507,
	["Delay"] = 0.1556839942932129,
	["PlayerRootPos"] = -1659.55859375, -28.036365509033203, 1639.5087890625
}

game:GetService("ReplicatedStorage").GunScripts.Events.GunShot:FireServer(ohTable1)

-- Script generated by SimpleSpy - credits to exx#9394

local args = {
    [1] = {
        ["HitPart"] = workspace.Shops.StableOwner2.HumanoidRootPart,
        ["EndPoint"] = Vector3.new(1494.4261474609375, 112.76438903808594, 1358.4443359375),
        ["HitCallback"] = "Bullet",
        ["BulletOwner"] = game:GetService("Players").LocalPlayer,
        ["Lifetime"] = 1,
        ["Speed"] = 80,
        ["cframe"] = CFrame.new(1519.222900390625, 115.30229187011719, 1399.2899169921875) * CFrame.Angles(-0.06205425411462784, 0.5447583794593811, 0.031709421426057816),
        ["Tool"] = game:GetService("Players").LocalPlayer.Character:FindFirstChild("Colt .45"),
        ...
        ["MaxDistance"] = 1000,
        ["Shot"] = workspace.Bullets.Shot,
        ["HitPosition"] = Vector3.new(1494.4261474609375, 112.76438903808594, 1358.4443359375),
        ["FirstFrame"] = true,
...
game:GetService("ReplicatedStorage").GunScripts.Events.Hit:FireServer(unpack(args))
...
do
	...
	local aimbot = main:Sector('silent aim','Right')
	aimbot:Element('ToggleKeybind','enabled')	
	aimbot:Element('Slider','FOV',{min = 0,max = 420,default = 120})
	aimbot:Element('Slider','hitchance',{min = 0,max = 100,default = 100})
	aimbot:Element('Dropdown','priority',{options = {'Head','UpperTorso','LowerTorso'}})
	aimbot:Element('ToggleTrans','draw fov', {default = {Color = COL3RGB(255,255,255), Transparency = 0}})
	aimbot:Element('Toggle','filled fov')
	
	aimbot:Element('Slider','fov thickness',{min = 1,max = 10,default = 1})
	RunService.Stepped:Connect(function()
		Fov.Visible = values.main['silent aim']['draw fov'].Toggle and values.main['silent aim'].enabled.Toggle

		Fov.Transparency = 1 - values.main['silent aim']['draw fov'].Transparency
		
		Fov.Color =  values.main['silent aim']['draw fov'].Color
		Fov.Position = Vec2(Mouse.X, Mouse.Y+36)
		Fov.Radius = values.main['silent aim'].FOV.Slider
		Fov.Thickness = values.main['silent aim']['fov thickness'].Slider
		Fov.Filled = values.main['silent aim']['filled fov'].Toggle		
	end)
	...
	local oldNamecall; oldNamecall = hookmetamethod(game,'__namecall', function(self, ...) 
		...
		local args = {...} 
		if self.Name == 'GunShot' and not checkcaller() and values.main['silent aim'].enabled.Toggle then
			if math.random(0,100) <= values.main['silent aim'].hitchance.Slider then
				lastBullet = args[1]['Shot']

				local player, nearestDistance = nil, values.main['silent aim'].FOV.Slider
					...
					if v ~= Players.LocalPlayer and v.Character and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health > 0 and v.Character:FindFirstChild("HumanoidRootPart") then
						...
						if visible then
							...
							if distance < nearestDistance then
								...
								player = v
							--[[if values.main['silent aim'].priority.Dropdown == 'closest' then
				...
				if player ~= nil then
					newCFrame = player.Character[values.main['silent aim'].priority.Dropdown]
					args[1]['HitHum'] = player.Character.Humanoid
					args[1]['HitPart'] = player.Character[values.main['silent aim'].priority.Dropdown]
					args[1]['EndPoint'] = player.Character[values.main['silent aim'].priority.Dropdown].Position
					args[1]['HitPosition'] = player.Character[values.main['silent aim'].priority.Dropdown].Position
					game:GetService("ReplicatedStorage").GunScripts.Events.Hit:FireServer(unpack(args))
				else
					newCFrame = nil
				end
	...
	local old; old = hookmetamethod(game,'__newindex',function(self,index,value)
		if self == lastBullet and index == 'CFrame' and newCFrame ~= nil then
			value = CFrame.lookAt(value.p,newCFrame.Position)
		end
...
do
	...
	do
		local gunClone,originalGun = nil,nil
		...
		for i,v in pairs(getgc(true)) do
			if typeof(v) == 'table' and rawget(v,"Colt .45") and rawget(v["Colt .45"],'FiringOffset') then
				gunClone = deepCopy(v)
				originalGun = v
			end
		end
		if originalGun == nil then
			local cache = require(game.ReplicatedStorage.GunScripts.GunStats)
			gunClone = deepCopy(cache)
			originalGun = cache
		end
		local gunMods = misc:Sector('gun mods','Right')
		gunMods:Element('Toggle','No recoil',{},function(tbl)
			for i,v in pairs(originalGun) do
				v.camShakeResist = (tbl.Toggle and 0 or gunClone[i].camShakeResist)
			end
		end)
		gunMods:Element('Toggle','No spread',{},function(tbl)
			for i,v in pairs(originalGun) do
				v.HipFireAccuracy = (tbl.Toggle and 0 or gunClone[i].HipFireAccuracy)
				v.ZoomAccuracy = (tbl.Toggle and 0 or gunClone[i].ZoomAccuracy)
			end
		end)	
		gunMods:Element('Toggle','Instant equip',{},function(tbl)
			for i,v in pairs(originalGun) do
				v.equipTime = (tbl.Toggle and 0 or gunClone[i].equipTime)
			end
		end)	
		gunMods:Element('Toggle','Instant reload',{},function(tbl)
			for i,v in pairs(originalGun) do
				v.ReloadSpeed = (tbl.Toggle and 0 or gunClone[i].ReloadSpeed)
			end
		end)			
	end
	...
	do
		...
		local old; old = hookmetamethod(game,'__namecall',function(self,...)
			...
			if self.Name == 'ChangeCharacter' then
				if (values.misc.misc.player['No enviroment damage'].Toggle and args[1] == 'Damage') or (values.misc.misc.player['No ragdoll'].Toggle and args[1] == 'Ragdoll') then
					return
				end
			end
...
--math.floor(value*100)/100
	...
	effects:Element("ToggleColor", "world color", {default = {Color = COL3RGB(255,255,255)}}, function(val) 
		...
		elseif Camera:FindFirstChild('ColorCorrection') then
			Camera:FindFirstChild('ColorCorrection'):Destroy()
		end 
	end) 
...
LocalPlayer.CharacterAdded:Connect(function(char) 
	...
	for i,v in pairs(char:GetChildren()) do 
		...
		elseif v:IsA("Accessory") and v.Handle.Transparency ~= 1 then 
			INSERT(SelfObj, v:WaitForChild('Handle')) 
			local Color = INST("Color3Value") 
			Color.Name = "OriginalColor" 
	...
	LocalPlayer.Character.ChildAdded:Connect(function(Child) 
		if Child:IsA("Accessory") and Child:WaitForChild('Handle').Transparency ~= 1 then 
