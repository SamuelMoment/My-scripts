...
function library:init()
    ...
    --
    utility:DoubleOutline(frame,Themes[Theme].Border,fromrgb(10,10,10))
    ...
    utility:DoubleOutline(backgroundTab,fromrgb(10,10,10),Themes[Theme].Border)
    -- Tabs handler
    ...
    utility:DoubleOutline(TabsHolder,Themes[Theme].Border,fromrgb(10,10,10))
    ...
    function library:AddTab(name)  
        ...
        utility:Outline(tab,Themes[Theme].Border)
        ...
        function sectorsFuncs:AddSection(options)
            ...
            print(section.AbsolutePosition  )
            ...
            function sectionFuncs:Toggle(options)
                ...
                utility:DoubleOutline(toggle,Themes[Theme].Border,fromrgb(10,10,10))
                ...
                function togglefuncs:ColorPicker(options)
                    ...
                    table.insert(additionals,button)
                    ...
                    utility:DoubleOutline(pickerHandler,Themes[Theme].Border,fromrgb(10,10,10))
                    --
                    --
                    --
                    --
                    ...
                    utility:DoubleOutline(colorpicker,Themes[Theme].Border,fromrgb(10,10,10))
                    ...
                    utility:Outline(alphaframe,Themes[Theme].Border)
                    ----------------------------------------------
                    ...
                    local col = {Color = {R = default.R,B = default.B, G = default.G},Transparency=alpha}
                    local function set(color,modifyPos)
                        ...
                        library.flags[flag] = {Color = color,Transparency = alpha}
                    end
                    set(default,true)
            ...
            function sectionFuncs:Dropdown(options)
                ...
                local default = (options.default and table.find(dropdownOptions,options.default)) and options.default or (dropdownOptions[1] and dropdownOptions[1] or 'NONE')
                ...
                local holder = utility:Draw('Square',{
                    ...
                    Visible = true,
                    Filled = true,
                    Position = UDim2.new(0,0,1,-10), -- fun fact: position doesnt matter because of list layout :rofl:
                ...
                local function update(index,shouldSet)
                    ...
                    if holder.Color == fromrgb(75,75,75) then -- is chosen/trying to remove?
                        --local next = #chosen - 1
                        if shouldSet then return end
                        if #chosen == min then
                            return
                        end
                        holder.Color = Themes[Theme].Object
                        local index = table.find(chosenHolders,holder)
                        table.remove(chosenHolders,index)
                        table.remove(chosen,index)
                        callback(max == 1 and chosen[1] or chosen)
                        library.flags[flag] = max == 1 and chosen[1] or chosen                    
                    else
                        if #chosen == max then
                            chosenHolders[1].Color = Themes[Theme].Object
                            table.remove(chosen,1)
                            table.remove(chosenHolders,1)
                        end
                        table.insert(chosen,name)
                        table.insert(chosenHolders,holder)
                        holder.Color = fromrgb(75,75,75)
                    end 
                ...
                for i,v in pairs(dropdownOptions) do
                    local holder = utility:Draw('Square',{
                        ...
                        Color = v == chosen and fromrgb(75,75,75) or Themes[Theme].Object,
                        Visible = true
                    })
                    --
                    if v == default then
                ...
                library.UpdateByFlag[flag] = function(newchosen)
                    if typeof(newchosen) == 'table' then
                        ...
                        update(table.find(dropdownOptions,newchosen),true)
                    end
                end
            ...
            function sectionFuncs:Separator(options)
                ...
                utility:DoubleOutline(left,Themes[Theme].Border,fromrgb(10,10,10))
...
function library:SaveConfig(name)
    local library = self -- it's easier for me to read library:AddTab rather than self:AddTab
    ...
    writefile('test.cfg',game.HttpService:JSONEncode(fakeFlags))
end
function library:LoadConfig(name)
    local library = self -- it's easier for me to read library:AddTab rather than self:AddTab

    local flags = game.HttpService:JSONDecode(readfile('test.cfg'))
...
library:init()
local tab1 = library:AddTab('Hi')
local section1 = tab1:AddSection({Name='Right',Side='Right'})

tab1:AddSection({Name='Test Left',Side='Left'})
tab1:AddSection({Name='Test Right',Side='Right'})
section1:Toggle{Name = 'Toggle 1',Flag = 'hi',callback = function(val)
    print("Toggle 1 is "..val)
end}
...
section1:Button{name = 'Button'}
section1:Dropdown{name = 'Dropdown',options = {'option 1','option 2'},max = 2}


local tab2 = library:AddTab('ab')
local section2 = tab2:AddSection({Name='Left',Side='Left'})
section2:Slider{Name = 'Slider 2',Min = 1,Max = 100,callback = function(val)
